...
open class ARViewController: UIViewController, ARTrackingManagerDelegate {
    ...
    open var closeButtonImage: UIImage? {
        didSet {
            self.closeButton?.setImage(self.closeButtonImage, for: .normal)
        }
    }
    ...
    */
    ...
    fileprivate func startCameraAndTracking(notifyLocationFailure: Bool) {
        ...
        self.displayTimer?.add(to: .current, forMode: .default)
    }
    ...
    func addCloseButton() {
        ...
        closeButton.addTarget(self, action: #selector(ARViewController.closeButtonTap), for: .touchUpInside)
    ...
    public static func isAllHardwareAvailable() -> NSError? {
        return CameraView.createCaptureSession(withMediaType: .video, position: AVCaptureDevice.Position.back).error
    }
    ...
    private func addDebugUi() {
        ...
        if self.uiOptions.debugMap {
            ...
            debugMapButton.addTarget(self, action: #selector(ARViewController.debugButtonTap), for: .touchUpInside)
            debugMapButton.setTitle("map", for: .normal)
            debugMapButton.backgroundColor = UIColor.white.withAlphaComponent(0.5)
            debugMapButton.setTitleColor(UIColor.black, for: .normal)
        ...
        if self.uiOptions.debugLabel {
            ...
            debugLabel.autoresizingMask = [.flexibleWidth, .flexibleTopMargin]
